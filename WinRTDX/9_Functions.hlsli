...
)
{
    ...
    float3 specularLight = pow(saturate(dot(reflectionVector, normalize(vectorToCamera))), specularPower) * specularColor;

    return saturate(materialColor * (diffusedLight + ambientLight) +  specularLight * attenuation);
};