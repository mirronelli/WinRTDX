...
namespace Dx::Drawables 
{
	...
	public:
		SphereColoredWithNormal (
			...
			m_steps(steps),
			mPixelPerInstanceConstants({1, 32})
		{}
		...
		void RegisterResources() {
			...
			m_psConstantBuffer = PSConstantBuffer<Specular>::					Create(m_resourceCacheID+1, false, mPixelPerInstanceConstants, (UINT)ResourceSlots::PerInstance);
		...
		void UpdateConstants(DirectX::CXMMATRIX matrix)
		{
			...
			std::static_pointer_cast<PSConstantBuffer<Specular >> (m_psConstantBuffer)->Update(mPixelPerInstanceConstants);
		}
	...
	private:
		...
		Specular									mPixelPerInstanceConstants;
